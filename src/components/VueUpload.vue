        ...
        methods: {
            initWebUpload() {
                this.uploader = WebUploader.create({
                    ...
                    swf: '/src/assets/webuploader/Uploader.swf',  // swf文件路径
                    ...
                    fileSingleSizeLimit: this.fileSingleSizeLimit,    // 单个文件大小限制
                    formData: this.formData,  // 上传所需参数
                    chunked: true,            //分片上传
                    chunkSize: 2 * 1024 * 1024,   //分片大小2M
                    duplicate: false,  // 重复上传
                    resize: this.resize //压缩
                });
                ...
                this.uploader.on('uploadStart', () => {
                    // 在这里可以准备好formData的数据
                    this.uploader.options.formData.uuid = getFakerData().UUID;
                    this.uploader.options.formData.datePath = getFakerData().datePath;
                });
